class Track < ActiveRecord::Base
  ...
  def get_slow_down_percentage
    begining_of_day = DateTime.now.in_time_zone(time_zone).beginning_of_day
    return 0 if zone_time.between?(begining_of_day + 9.hours, begining_of_day + 18.hours)
    return 15 if zone_time.between?(begining_of_day + (21.hours + 30.minutes + 1.second), begining_of_day + 1.day + 6.hours)
    8
  end
end