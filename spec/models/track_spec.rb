...
RSpec.describe Track, type: :model do
  ...
  describe ".slow_down_for_zone" do
    let(:snow_track) { build :track }
    
    it "should return the slow_down_for_zone when current driving time is between 9am to 6pm " do
      allow_any_instance_of(Date).to receive(:in_time_zone).and_return(DateTime.parse("5am"))
      expect(snow_track.slow_down_for_zone(100, 'CET')).to eql(0.0)
    end

    it "should return the slow_down_for_zone when current driving time is between 6pm to 9:30pm " do
      allow_any_instance_of(Date).to receive(:in_time_zone).and_return(DateTime.parse("8pm"))
      expect(snow_track.slow_down_for_zone(100, 'CET')).to eql(8.0)
    end

    it "should return the slow_down_for_zone when current driving time is between 9:30pm to 6am " do
      allow_any_instance_of(Date).to receive(:in_time_zone).and_return(DateTime.parse("10pm"))
      expect(snow_track.slow_down_for_zone(100, 'CET')).to eql(15.0)
    end

    it "should return the slow_down_for_zone when current driving time is between 6am to 9:30pm " do
